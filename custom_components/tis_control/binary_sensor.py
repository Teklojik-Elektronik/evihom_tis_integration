...
async def async_setup_entry(hass,entry,async_add_entities):
    ...
    if B:
        C=[(C,next(iter(A[alpha__("Y2hhbm5lbHM=")][0].values())),A[alpha__("ZGV2aWNlX2lk")],A[alpha__("Z2F0ZXdheQ==")],A[alpha__("aXNfcHJvdGVjdGVk")],A.get(alpha__("ZGV2aWNlX25hbWU="),"Unknown"),A.get(alpha__("ZGV2aWNlX3R5cGU="),"Unknown"))for B in B for(C,A)in B.items()]
        D=[TISBinarySensor(tis_api=A,sensor_name=B,channel_number=C,device_id=D,gateway=E,device_name=F,device_type=G)for(B,C,D,E,H,F,G)in C]
    async_add_entities(D)
class TISBinarySensor(BinarySensorEntity):
    def __init__(A,tis_api,sensor_name,channel_number,device_id,gateway,device_name="Unknown",device_type="Unknown"):
        A._api=tis_api
        A._name=sensor_name
        A._device_id=device_id
        A._channel_number=int(channel_number)
        A._listener=_A
        A._attr_state=_A
        A._attr_is_on=_A
        A._attr_device_class=alpha__("bW90aW9u"),
        A._gateway=gateway
        A._device_name=device_name
        A._device_type=device_type
        A._attr_unique_id=beta__("e19fdmFyMH1fe19fdmFyMX0=", __var0=A._name, __var1=A._channel_number)
    ...
    def device_info(self):
        return DeviceInfo(
            identifiers={("tis_control", str(self._device_id))},
